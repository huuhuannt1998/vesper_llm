...
try:
    ...
    print("✅ Position mapping system available")
...
class BGENavigationMapper:
    ...
    def _initialize_mapper(self):
        ...
        try:
            ...
            if house_layout_path:
                ...
                print("✅ BGE Navigation Mapper initialized")
            else:
                print("⚠️ House layout not found - using fallback mapping")
                self.mapper = VESPERPositionMapper()
                
        except Exception as e:
            print(f"❌ Failed to initialize position mapper: {e}")
            self.mapper = None
        ...
        try:
            ...
            if should_generate_map:
                ...
                if map_path:
                    ...
                    print(f"🗺️ Updated navigation map: {os.path.basename(map_path)}")
                    return map_path
            
        except Exception as e:
            print(f"❌ Error updating position map: {e}")
    ...
    def generate_full_history_map(self):
        ...
        try:
            ...
            print(f"❌ Error generating history map: {e}")
            return None
...
def test_bge_integration():
    ...
    print("🧪 Testing BGE Navigation Mapping Integration")
    
    mapper = get_navigation_mapper()
    
    if not mapper.mapper:
        print("❌ Mapper not available for testing")
        return
    ...
    for i, (x, y, room, task, target) in enumerate(test_sequence):
        print(f"\n📍 Step {i+1}: Actor at ({x:.2f}, {y:.2f}) in {room}")
        
        map_path = update_actor_position_map(x, y, room, task, target)
        
        if map_path:
            print(f"  🗺️ Generated map: {os.path.basename(map_path)}")
        
        # Simulate task completion
        if room == target and i == len(test_sequence) - 1:
            print(f"✅ Task '{task}' completed in {room}")
            summary_map = generate_session_summary_map()
            if summary_map:
                print(f"📊 Session summary map: {os.path.basename(summary_map)}")
