#!/usr/bin/env python3
"""
BGE Navigation Startup Diagnostic
Identifies why the navigation script isn't initializing properly after BGE starts
"""

import os

def check_bge_initialization_issues():
    """Check for common BGE initialization problems"""
    print("🔍 BGE Navigation Startup Diagnostic")
    print("=" * 50)
    
    print("📋 ISSUE: BGE starts but navigation script doesn't initialize")
    print("   - Game Engine Started ✅")
    print("   - Navigation script initialization ❌") 
    print("   - Game Engine Finished ✅")
    print("   - Result: Empty log with all zeros")
    
    print("\n🔍 COMMON INITIALIZATION FAILURES:")
    
    issues = [
        "1. Import Error - BGE can't import required modules",
        "2. Logic Brick Not Connected - Script not attached to Actor",
        "3. Script Path Wrong - Blender can't find llm_bge_navigation.py", 
        "4. Actor Object Missing - No 'Actor' object in scene",
        "5. Python Script Syntax Error - Compilation failure",
        "6. Missing Dependencies - backend.app.llm modules not found",
        "7. Early Exception - Script crashes before metrics initialization"
    ]
...
def create_minimal_test_script():
    ...
    print("\n🛠️ CREATING MINIMAL TEST SCRIPT:")
...
'''
    ...
    test_file = r"c:\Users\hbui11\Desktop\vesper_llm\blender\minimal_bge_test.py"
...
def provide_next_steps():
    ...
    steps = [
        "1. 🔄 REPLACE BGE SCRIPT TEMPORARILY:",
        "   • In Blender Logic Editor, change Python script",
        "   • From: llm_bge_navigation.py", 
        "   • To: minimal_bge_test.py",
        "",
        "2. 🎮 TEST WITH MINIMAL SCRIPT:",
        "   • Press P to start BGE",
        "   • Watch Blender console carefully",
        "   • Look for the test output messages",
        "   • Note any ❌ failures",
        "",
        "3. 🔍 ANALYZE RESULTS:",
        "   • If minimal test works → Original script has import/syntax issues",
        "   • If minimal test fails → BGE setup problem (Actor missing, logic bricks)",
        "   • If no output at all → Script not connected to logic bricks",
        "",
        "4. 🛠️ BASED ON RESULTS:",
        "   • Success: Fix original script imports/syntax",
        "   • Failure: Fix BGE setup (Actor object, logic bricks)",
        "   • No output: Fix logic brick connections",
        "",
        "5. 🔙 RESTORE ORIGINAL:",
        "   • After diagnosis, change script back to llm_bge_navigation.py",
...
def main():
    ...
    check_bge_initialization_issues()
    ...
    if create_minimal_test_script():
        ...
        print("• Empty log = BGE starts but navigation script fails to initialize")
        print("• Use minimal_bge_test.py to isolate the problem")
        print("• Check Blender console for detailed error messages")
        print("• Most likely causes: import errors or missing Actor object")
