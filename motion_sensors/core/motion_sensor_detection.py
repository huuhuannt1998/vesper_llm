...
class MotionSensorDetector:
    ...
    def trigger_motion_sensor(self, sensor_id: str, trigger_type: str = "motion_detected"):
        ...
        # Update visual detection area if available
        try:
            self.update_visual_detection_area(sensor_id, trigger_type == "motion_detected")
        except Exception as e:
            print(f"⚠️ Failed to update visual area: {e}")
    ...
    def update_visual_detection_area(self, sensor_id: str, detecting: bool):
        ...
        """
        try:
            # Get the VESPER device manager to update visual areas
            scene = bge.logic.getCurrentScene()
            if hasattr(scene, 'vesper_device_manager'):
                device_manager = scene.vesper_device_manager
                if hasattr(device_manager, 'update_detection_area_visualization'):
                    device_manager.update_detection_area_visualization(sensor_id, detecting)
        except Exception as e:
            # Visual updates are optional - don't fail sensor operation
            pass
