...
Updated with proper first-person camera system for VLM decision-making.
"""
...
# Add actor camera system
blender_dir = os.path.join(os.path.dirname(__file__), '..', 'blender')
sys.path.append(blender_dir)

try:
    from actor_camera_system import VESPERActorCamera, capture_vlm_decision_context
    CAMERA_SYSTEM_AVAILABLE = True
except ImportError:
    print("Warning: Actor camera system not available")
...
# ============================================================================
# ENHANCED TOOL: VLM DECISION SUPPORT WITH FIRST-PERSON CAMERA
# ============================================================================

@mcp.tool()
def setup_actor_first_person_camera(actor_name: str = "Actor") -> Dict[str, Any]:
    """
    Set up first-person camera attached to the actor for VLM visual input.
    Call this once to initialize the camera system.
    
    Args:
        actor_name: Name of the actor object in Blender
        
    Returns:
        Setup status and camera information
    """
    try:
        if not CAMERA_SYSTEM_AVAILABLE:
            return {"error": "Camera system not available"}
        
        camera_system = VESPERActorCamera(actor_name)
        success = camera_system.setup_actor_camera()
        ...
        if success:
            return {
                ...
                "camera_height": 1.6,
                "field_of_view": 75,
                "message": "First-person camera ready for VLM decision-making"
            }
        else:
            return {"error": "Failed to set up camera system"}
            
    except Exception as e:
        return {"error": f"Camera setup failed: {str(e)}"}

@mcp.tool()
def capture_vlm_decision_context(actor_name: str = "Actor") -> Dict[str, Any]:
    """
    Capture complete context for VLM decision-making when uncertain about next action.
    Provides both first-person visual view and comprehensive spatial information.
    
    Args:
        actor_name: Name of the actor object
        
    Returns:
        Complete visual and spatial context for VLM analysis
    """
    try:
        if not CAMERA_SYSTEM_AVAILABLE:
            return {"error": "Camera system not available"}
        
        # Get complete decision context
        context = capture_vlm_decision_context(actor_name)
        
        # Add VLM-specific guidance
        context["vlm_guidance"] = {
            "when_to_use": [
                "When uncertain about which direction to move",
                "When need to identify objects or rooms",
                "When need to plan navigation route",
                "When task requires visual confirmation"
            ],
            "decision_process": [
                "1. Analyze first-person view for visible objects/rooms",
                "2. Check coordinate position relative to house layout", 
                "3. Consider available movement directions",
                "4. Plan action based on task requirements"
            ],
            "available_actions": [
                "execute_movement_action() - Move in available direction",
                "execute_interaction_action() - Interact with visible object",
                "capture_vlm_decision_context() - Get updated context",
                "get_room_connectivity_map() - Plan multi-room navigation"
            ]
        }
        ...
        return {"error": f"Decision context capture failed: {str(e)}"}

@mcp.tool()
def get_actor_first_person_view(actor_name: str = "Actor", 
                               resolution_width: int = 1024,
                               resolution_height: int = 768) -> Dict[str, Any]:
    """
    Get just the first-person view from the actor's camera.
    Lighter weight than full decision context when only visual is needed.
    ...
    Returns:
        First-person view image and basic metadata
    """
    try:
        if not CAMERA_SYSTEM_AVAILABLE:
            return {"error": "Camera system not available"}
        ...
        camera_system = VESPERActorCamera(actor_name)
        result = camera_system.capture_first_person_view((resolution_width, resolution_height))
        
        # Add usage guidance
        result["usage_notes"] = {
            "purpose": "Visual context for VLM when uncertain about environment",
            "complements": "Use with get_spatial_context() for full awareness",
            "best_for": "Object recognition, room identification, obstacle detection"
        }
        
        return result
        
    except Exception as e:
        return {"error": f"First-person view capture failed: {str(e)}"}

# ============================================================================
# CORE TOOL 1: MULTI-VIEW IMAGE ANALYSIS (UPDATED)
# ============================================================================
